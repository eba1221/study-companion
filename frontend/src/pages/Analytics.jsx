...
const SUBJECT_COLORS = {
  ...
  Other: { fill: "rgba(200,200,200,0.85)", border: "rgba(160,160,160,0.5)", text: "#555" },
};
...
// Bar chart of study time in last 7 days
      ...
      {data.map((d, i) => (
        ...
        >
          <div style={{ flex: 1, width: "100%", display: "flex", alignItems: "flex-end" }}>
            {/* Height of each bar represents minutes studied */}
            <div
              title={`${d.mins} min`}
              style={{
                width: "100%",
                height: `${Math.max((d.mins / max) * 100, d.mins > 0 ? 6 : 2)}%`,
                borderRadius: "10px 10px 6px 6px",
                background:
                  d.mins > 0
                    ? "linear-gradient(180deg,rgba(58,30,16,0.55),rgba(58,30,16,0.28))"
                    : "rgba(58,30,16,0.08)",
...
// Line chart of predicted score trajectory
function TrajectoryChart({ data }) {
  if (!data?.length) return null;

  // Dimensions and scales for SVG coordinate system
  const W = 560;
  const H = 190;
  const P = { t: 16, r: 16, b: 32, l: 38 };
  ...
  // Split past and future points
  const past = data.filter((d) => d.isPast);
  const future = data.filter((d) => !d.isPast);

  const toPath = (pts) =>
    pts.map((d, i) => `${i === 0 ? "M" : "L"}${sx(data.indexOf(d))},${sy(d.predictedScore)}`).join(" ");

  const fp =
    future.length && past.length
      ? `M${sx(data.indexOf(past[past.length - 1]))},${sy(past[past.length - 1].predictedScore)} ` +
        future.map((d) => `L${sx(data.indexOf(d))},${sy(d.predictedScore)}`).join(" ")
      : "";
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto" }}>
      ...
      {data.map((d, i) => (
        <g key={i}>
          <circle
            cx={sx(i)}
            cy={sy(d.predictedScore)}
            r={d.isPast ? 4 : 3}
            fill={d.isPast ? "rgba(58,30,16,0.72)" : "rgba(255,255,255,0.95)"}
            stroke="rgba(58,30,16,0.45)"
            strokeWidth={1.5}
          />
          {!d.isPast && (
            <text
              x={sx(i)}
              y={sy(d.predictedScore) - 8}
              fontSize={9}
              fontWeight={800}
              fill="rgba(58,30,16,0.50)"
              textAnchor="middle"
            >
              {d.predictedScore}%
            </text>
...
// Skeleton loader shows a loading placeholder while the analytics data is being fetched
  ...
  return (
    <div
      style={{
        ...
        borderRadius: r,
        background:
          "linear-gradient(90deg,rgba(58,30,16,0.07) 25%,rgba(58,30,16,0.13) 50%,rgba(58,30,16,0.07) 75%)",
...
export default function AnalyticsPage() {
  ...
  // State variables for analytics data, loading status, and error handling
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Function to fetch analytics predictions from the server
  async function fetchPredictions() {
    setLoading(true);
    setError(null);

    try {
      // Get stored user token for authentication
      ...
      // Send POST request to analytics endpoint with authentication token
      ...
      // Update state with fetched analytics data
  ...
  // Fetch analytics predictions when the component mounts
  useEffect(() => {
    fetchPredictions();
  }, []);

  // Extract key performance indicators from fetched data
  const kpis = data?.kpis ?? {};
  const weakTopics = data?.weakTopics ?? [];
  const subjects = data?.subjectStats ?? [];

  // Prepare study time data for the last 7 days
  const studyChart = useMemo(() => {
    if (data?.studyChart) return data.studyChart;

    // Generate default study time data for the last 7 days with 0 minutes studied
    const DAY_LABELS = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
    return Array.from({ length: 7 }, (_, i) => {
      const d = new Date(Date.now() - (6 - i) * 86400000);
      return {
        label: DAY_LABELS[d.getDay() === 0 ? 6 : d.getDay() - 1] + " " + d.getDate(),
        mins: 0,
      };
    });
  }, [data]);
  ...
  return (
    <div className="learnShell">
      ...
      <aside className="sidebar">
        ...
        <Link className="navBtn" to="/learn">
          <BookOpen size={22} />
        </Link>
        <Link className="navBtn navBtnActive" to="/analytics">
          <BarChart3 size={22} />
        </Link>
        <div style={{ flex: 1 }} />
      </aside>
      ...
      <main className="main">
        ...
        <div className="card aHero">
          <div className="aHeroRow">
            <div>
              ...
              <p className="aHeroSub">
                Predictions trained on 2,000 synthetic GCSE student profiles — tuned to your real quiz history.
              </p>
            </div>
        ...
        <div className="aKpis">
          <div className="card aKpi">
            <div className="aKpiTop">
              <span className="aKpiLabel">Study time</span>
            ...
            {loading ? (
              ...
              <div className="aKpiValue">
                {kpis.weeklyStudyTimeMins >= 60
                  ? `${(kpis.weeklyStudyTimeMins / 60).toFixed(1)}h`
                  : `${kpis.weeklyStudyTimeMins ?? 0}m`}
              </div>
            )}
          ...
          <div className="card aKpi">
            <div className="aKpiTop">
              <span className="aKpiLabel">Accuracy</span>
              <Target size={16} style={{ opacity: 0.5 }} />
            </div>
          ...
          <div className="card aKpi">
            <div className="aKpiTop">
              <span className="aKpiLabel">Trend</span>
            ...
            {loading ? (
              ...
              <div
                ...
                style={{
                  color:
                    kpis.trend == null ? "var(--ink)" : kpis.trend >= 0 ? "#16a34a" : "#dc2626",
                }}
              >
          ...
          <div className="card aKpi" style={{ cursor: "pointer" }} onClick={fetchPredictions}>
            <div className="aKpiTop">
              <span className="aKpiLabel">Refresh</span>
              <RefreshCw size={16} style={{ opacity: 0.5 }} />
            </div>
            <div className="aKpiValue" style={{ fontSize: 14, opacity: 0.6 }}>
              Update
            </div>
        ...
        {/* Grid container for subject accuracy and weak topics panels */}
        <div className="aGrid">
          {/* Subject accuracy */}
          <div className="card aPanel">
            ...
            <div className="aPanelScroll">
              {loading
                ? [1, 2, 3].map((i) => <Sk key={i} h={44} r={12} />)
                : subjects.map((s) => (
                    <div
                      key={s.subject}
                      style={{
                        ...
                        padding: "12px 14px",
                        borderRadius: 14,
                        border: `1px solid ${sc(s.subject, "border")}`,
                        background: sc(s.subject, "fill").replace("0.85", "0.12"),
                      }}
                    >
                      <div
                        style={{
          ...
          {/* Display a list of weak topics with priority levels */}
          <div className="card aPanel">
            <div className="aPanelHeader">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          style={{
                            fontSize: 11,
                            color: "rgba(58,30,16,0.58)",
                            fontWeight: 700,
                          }}
                        >
                          {t.subject} · {t.weightedScore}% · {t.attempts} attempt
                          {t.attempts !== 1 ? "s" : ""}
                        </div>
        ...
        {/* Container for the score trajectory and study time charts */}
